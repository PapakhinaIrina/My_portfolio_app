...
import { DayShowComponentContainer, 
  ...
  return (
    <> 
      <DayShowComponentContainer disableGutters>
          { currentDayEvents.length > 0 && cells.map((eventsList, index) => {
            return (
              <DayShowComponentWrapper>
                {
                  index ? (
                    <Box sx={{height: '60px'}} key={uuidv4()}>
                ...
                <DayShowComponentScaleItem>
                  {
                    eventsList.length > 0 && eventsList.map((event) => {
                      return (
                        <DayShowComponentScaleItemText onClick={() => handleChosenEvent(event)}>
                          <Box sx={{textTransform: "capitalize", textOverflow: "ellipsis", overflow: "hidden", whiteSpace: "nowrap", padding: '2px'}}>
                            <div>{event.title}</div>
                          </Box>
                          <Box sx={{ textTransform: "capitalize", textOverflow: "ellipsis", overflow: "hidden", whiteSpace: "nowrap", padding: '2px'}}>
